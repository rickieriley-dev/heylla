...
const broadcastViewerCount = async (io, roomId) => {
  ...
  // Also update the homepage lobby with live viewer counts
  io.emit('lobby:viewers_updated', { roomId, count });
};

// Fetch all active rooms and push to everyone on the homepage
const broadcastLobbyRooms = async (io) => {
  try {
    const Room = require('../models/Room');
    const rooms = await Room.findAll();
    io.emit('lobby:rooms_updated', rooms);
  } catch (e) {
    console.error('broadcastLobbyRooms error:', e);
  }
};
...
module.exports = (io, socket) => {
  ...
  socket.on('room:join', async ({ roomId }) => {
    ...
    let [seats, room, adminIds] = await Promise.all([
      Seat.getSeats(rid),
      Room.findById(rid),
      getAdminIds(rid),
    ]);

    // Room not found — notify client and bail
    if (!room) {
    ...
    // Auto-activate if host is rejoining their own inactive room
    if (!room.is_active) {
      if (room.host_id === socket.user.id) {
        await db.query('UPDATE rooms SET is_active=true WHERE id=$1', [rid]);
        room.is_active = true;
      } else {
        socket.emit('room:error', { message: 'This room has ended.' });
        socket.leave(rid);
        socket.currentRoom = null;
        return;
      }
